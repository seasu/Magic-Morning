...
| v3.2.0 | 2026-03-12 | **流程重設計**：(1) Editor 新增 `EditorPhase`（confirm / swiping）；Spec 完成後進入「確認預覽」畫面，PageView 瀏覽 8 張素材預覽，智能 CTA 按鈕根據點數顯示「生成全部 8 張」或「生成前 N 張」；確認後批次自動生成，移除逐張手動點擊。(2) 登入獎勵邏輯從 Flutter 端移至 Cloud Function `promoteUser`，透過 `LOGIN_BONUS_CREDITS` defineString 控制；`auth_service._promoteUser()` 改呼叫 CF，點數由 server 原子性扣發並寫入 creditHistory。 |
